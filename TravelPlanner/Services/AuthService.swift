...
class AuthService {
    ...
    // ✅ Hàm xác thực OTP
    static func verifyOTP(email: String, code: String, completion: @escaping (_ success: Bool, _ message: String, _ token: String?, _ firstName: String?, _ lastName: String?, _ username: String?, _ shouldGoToHome: Bool) -> Void) {
            guard let url = URL(string: "https://travel-api-79ct.onrender.com/api/v1/auth/email-verify-otp") else {
                completion(false, "URL xác thực OTP không hợp lệ.", nil, nil, nil, nil, false)
                return
            }
            ...
            do {
                ...
                completion(false, "Lỗi mã hóa dữ liệu OTP: \(error.localizedDescription)", nil, nil, nil, nil, false)
                return
            }
            ...
            session.dataTask(with: request) { data, response, error in
                DispatchQueue.main.async {
                    if let error = error {
                        completion(false, "Lỗi kết nối: \(error.localizedDescription)", nil, nil, nil, nil, false)
                        return
                    }
                    
                    guard let httpResponse = response as? HTTPURLResponse else {
                        completion(false, "Không có phản hồi từ server.", nil, nil, nil, nil, false)
                        return
                    }
                    
                    guard (200...299).contains(httpResponse.statusCode), let data = data else {
                        completion(false, "Xác thực OTP thất bại. Mã lỗi: \(httpResponse.statusCode)", nil, nil, nil, nil, false)
                        return
                    }
                    ...
                    do {
                        if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            ...
                            if let dataDict = json["data"] as? [String: Any] {
                                ...
                                if success {
                                    completion(true, message, token, firstName, lastName, username, shouldGoToHome)
                                    print("Token: \(token ?? "Không nhận được token")")
                                } else {
                                    completion(false, message, nil, nil, nil, nil, false)
                                }
                            } else {
                                completion(false, "Phản hồi không hợp lệ từ server.", nil, nil, nil, nil, false)
                            }
                        }
                    } catch {
                        completion(false, "Lỗi phân tích phản hồi: \(error.localizedDescription)", nil, nil, nil, nil, false)
                    }
                }
            }.resume()
        }
